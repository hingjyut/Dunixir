defmodule Identity.Augmentation.ExcludedIsMember do
  ...
  setup_all do
    ...
    :lists.foreach(
      fn {iindex, mindex} ->
        Index.Generation.add_identity("blockstamp_test", iindex, mindex, %{
          "pub" => "pub_toto",
      ...
      [{local_iindex_a, local_mindex_a}, {local_iindex_b, local_mindex_b}]
    )
    ...
    :lists.foreach(
      fn index ->
        [{"pub_toto", elt}] = :dets.lookup(index, "pub_toto")
    ...
    {:ok,
     id_a: a_id.excludedIsMember,
     excl_a: a_excl.excludedIsMember,
     id_b: b_id.excludedIsMember,
     excl_b: b_excl.excludedIsMember}
  end
